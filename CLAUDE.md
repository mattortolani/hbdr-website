...
HBDR Website is a server-rendered marketing site for HBDR, an ad-tech company specializing in header bidding and publisher monetization. It includes 16+ pages, a blog with CMS admin panel, and a contact form with lead capture.
...
server/
  ...
  template.ts       # ALL page templates (3363 lines, monolithic)
shared/
...
- **Dual entry points**: `server/index.ts` for Node.js dev, `worker.ts` for Cloudflare Workers. Routes are **duplicated** in both (this is a known issue).
- **CDN dependencies**: Tailwind, Alpine.js, HTMX, DaisyUI, and Inter font are all loaded from CDNs. No frontend build step.

## Common Gotchas

1. **worker.ts duplicates routes.ts** -- Any route change MUST be made in both files or it will only apply to one environment.
2. **template.ts is 3363 lines** -- All 16+ page templates live in one file. Use search, not scrolling.
...
9. **Footer/social links go to `#`** -- Social icons, Resources, and Legal links are placeholder `href="#"`.
10. **OG image URLs are relative** -- Social media previews won't show images because `og:image` uses `/assets/...` instead of absolute URL.
...
### Done
- All 16+ page templates (Homepage, About, How It Works, Careers, Press, Contact, 9 Solution pages, Blog listing, Blog post view, Blog admin)
- Navigation with Solutions dropdown (desktop + mobile)

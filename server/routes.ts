...
function sanitizeHtml(html: string): string {
  const allowedTags = ['h1','h2','h3','h4','h5','h6','p','a','ul','ol','li','strong','em','b','i','br','blockquote','pre','code','img','span','div','hr','table','thead','tbody','tr','th','td'];
  const allowedAttrs = ['href','src','alt','class','target','rel','width','height'];
  let result = html
...
export function registerRoutes(app: Hono) {
  ...
  app.post("/api/blog", async (c) => {
    try {
      ...
      if (existing) {
        return c.json({ message: "A post with this slug already exists" }, 400);
      }
